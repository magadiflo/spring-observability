...
public class OrderController {
    ...
    // Inyectamos MeterRegistry (registro central de métricas)
...
- Creamos un `Counter` con nombre `orders_created_total`.
...
````bash
# HELP orders_created_total_total Total de órdenes creadas
# TYPE orders_created_total_total counter
orders_created_total_total 3.0
````

> 🔎 Nota: `Micrometer` agrega el sufijo `_total` automáticamente para los `Counter`, siguiendo la convención de
> `Prometheus`.
