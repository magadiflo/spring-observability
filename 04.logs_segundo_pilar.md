...
<configuration>
    ...
    <Appenders>
        ...
        <RollingFile name="FILE"
                     fileName="${LOG_PATH}/${APP_NAME}.log"
                     filePattern="${LOG_PATH}/${APP_NAME}_%d{yyyy-MM-dd}.log">
            <JsonTemplateLayout eventTemplateUri="classpath:custom-layout.json">
...
- `eventTemplateUri="classpath:custom-layout.json"`: Usa nuestro template personalizado `custom-layout.json`.
...
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Tue, 16 Sep 2025 22:48:40 GMT
<
{
  "orderId": "906c1c4b-9935-4e0a-a91a-17f0e4319dd7",
  "product": "Tablet",
  "price": 10,
  "quantity": 10
}
...
< HTTP/1.1 400
< Content-Length: 0
< Date: Tue, 16 Sep 2025 22:49:08 GMT
< Connection: close
<
````

````bash
$ curl -v http://localhost:8080/api/v1/orders/01010101 | jq
>
< HTTP/1.1 404
< Content-Length: 0
< Date: Tue, 16 Sep 2025 22:49:31 GMT
````
...
< Content-Type: application/json
< Transfer-Encoding: chunked
< Date: Tue, 16 Sep 2025 22:49:52 GMT
<
[
  {
    "orderId": "906c1c4b-9935-4e0a-a91a-17f0e4319dd7",
...
Al revisar el archivo `spring-observability.log`, vemos que los registros `ya no están en texto plano`, sino en
`formato JSON` estructurado. Esto se debe a que configuramos el layout con nuestro template `custom-layout.json`,
el cual define explícitamente los campos a mostrar.

````bash
...
{"timestamp":"2025-09-16T17:49:31.292-0500","level":"INFO","thread":"http-nio-8080-exec-4","logger":"dev.magadiflo.observability.app.controller.OrderController","message":"Buscando orden con ID: 01010101","service":"spring-observability"}
{"timestamp":"2025-09-16T17:49:31.293-0500","level":"WARN","thread":"http-nio-8080-exec-4","logger":"dev.magadiflo.observability.app.controller.OrderController","message":"Orden con ID: 01010101 no encontrada","service":"spring-observability"}
{"timestamp":"2025-09-16T17:49:52.081-0500","level":"INFO","thread":"http-nio-8080-exec-6","logger":"dev.magadiflo.observability.app.controller.OrderController","message":"Obteniendo todas las órdenes. Total actual: 1","service":"spring-observability"}
````

🔑 `Conclusión`: mantenemos la misma aplicación y endpoints, pero ahora los logs son generados con `Log4j2`
en `formato JSON`, listos para ser consumidos y filtrados de forma estructurada en `Loki + Grafana`.

## FASE 2B | Paso 5 – Visualización en Grafana
...
![01.png](assets/fase2b/01.png)

- Filtrar solo logs de nivel `ERROR` usando el campo `JSON` llamado `level`:
  `{app="spring-observability"} | json | level=~"ERROR|WARN"`

![02.png](assets/fase2b/02.png)

- Filtrar logs por campo del json: `{app="spring-observability"} | json | service="spring-observability"`

![03.png](assets/fase2b/03.png)

- Filtrar por logger específico:
  `{app="spring-observability"} | json | logger="dev.magadiflo.observability.app.controller.OrderController"`

![04.png](assets/fase2b/04.png)
